...
<?php
class Coach {
	protected static $table_name="coach";
	public $coachid;
	public $email;
	public $password;
	public $first_name;
  ...
  public static function authenticate($email="", $password="") {
    global $database;
	//This function check if the coach exists in the database
  ...
  public static function find_by_id($coachid=0) {
  ...
  public function full_name() {
	...
	public function save() {
	  ...
	  if(isset($this->coachid)){
		...
		$result_array = static::find_by_sql("SELECT coachid FROM ".self::$table_name." ORDER BY coachid DESC limit 1");
		if(empty($result_array)){
			$this->coachid = 1;
		}else{
			$this->coachid =array_shift($result_array)->coachid+1;
		}
		return $this->create();
	  }
	}
	...
	public function create() {
	  ...
	  if($database->query($sql)) {
	    $this->coachid = $database->insert_id();
	    return true;
	  } else {
	    return false;
	  }
	...
	public function update() {
	  global $database;
		...
		$sql .= " WHERE coachid=". $database->escape_value($this->coachid);
	  $database->query($sql);
