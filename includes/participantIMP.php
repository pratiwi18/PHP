...
<?php
class participantIMP {
	protected static $table_name="participant_impairments";
  ...
  public static function find_by_sql($sql="") {
	...
	// If its in the database there is not any need to update or change anything
	  $result_array = static::find_by_sql("SELECT participantid FROM ".self::$table_name." WHERE participantid={$this->participantid} AND imp_id='{$this->imp_id}' LIMIT 1");
	...
	public static function delete_by_impids($participantid,$impids){
		global $database;
		
		$sql = "DELETE FROM ".self::$table_name." WHERE participantid={$participantid} AND imp_id in ";
		$sql.= "('".implode("','",array_values($impids))."')";
		$database->query($sql);
	}

    public static function delete_by_participantid($participantid){
        global $database;
