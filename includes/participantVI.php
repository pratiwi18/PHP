...
class participantVI
{
    protected static $table_name = "participant_value_identification";
    ...
    public static function find_by_pid_latest($participantid = 0)
    {
        $result_array = static::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE participantid={$participantid}  order by `timestamp` desc LIMIT 1");
        return !empty($result_array) ? array_shift($result_array) : false;
    }
    ...
    public function save()
    {
        ...
        $result_array = static::find_by_sql("SELECT participantid FROM " . self::$table_name . " WHERE participantid={$this->participantid} LIMIT 1");
